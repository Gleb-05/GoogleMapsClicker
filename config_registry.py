...
@dataclass
class LoadFromJsonMixin:
    '''
    Use this class as base class to enable updating a config singelton-like object from a json dump file.
    Utilises dacite.
    ...
    ```
    class Config(DaciteLoadMixin):
        DACITE_CAST_TYPES = (Enum,)  # remember that one-element tuples require a comma
        # ...

    ```

    *See load_config_from_dict and load_preferences as examples of setting up the update.
    Single `_update(data : dict)` is implemented here. Not to be called outside of atomic load operations.*
    '''
...
@dataclass()
class ConfigRegistryMixin(LoadFromJsonMixin):
    """
